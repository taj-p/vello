...
/// Gamma correction lookup tables for text rendering (generated by build.rs).
mod gamma {
...
pub fn render(
    ...
    dispatch!(level, simd => render_impl(simd, tiles, strip_buf, alpha_buf, fill_rule, aliasing_threshold, paint_luminance, lines));
}
...
impl GammaCorrector {
    fn new(luminance: u8) -> Self {
        let lo_idx = find_anchor_index(luminance);
...
fn render_impl<S: Simd>(
    ...
    let gamma_corrector = paint_luminance.map(GammaCorrector::new);
...
mod tests {
    ...
    fn fixed_point_anchor_index_is_correct() {
        ...
        for luminance in 0..=255u8 {
            let fixed_point_idx = super::find_anchor_index(luminance);
